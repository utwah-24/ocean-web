...
export function AddProduct() {
  ...
  useEffect(() => {
    ...
    // Fetch the actual seller ID for this user
    const fetchSellerId = async () => {
      try {
        // First, try to get seller_id from user object
        const userSellerId = (user as any).seller_id || (user as any).sellerId || (user as any).seller?.id;
        
        if (userSellerId) {
          console.log('[AddProduct] Using seller_id from user:', userSellerId);
          setSellerId(userSellerId);
          return;
        }

        // Try using user.id as seller_id directly (common pattern)
        console.log('[AddProduct] Attempting to fetch seller with user.id:', user.id);
        try {
          const sellerData = await apiService.getSeller(user.id);
          
          // Verify this seller belongs to the logged-in user
          if (sellerData.user_id === user.id) {
            console.log('[AddProduct] Found seller ID:', sellerData.id);
            setSellerId(sellerData.id);
            
            // Update user object in auth context with seller_id for future use
            const updatedUser = { ...user, seller_id: sellerData.id };
            localStorage.setItem('user', JSON.stringify(updatedUser));
            return;
          }
        } catch (directErr) {
          console.log('[AddProduct] Direct lookup failed, searching by user_id');
        }
        ...
        if (mySeller) {
          console.log('[AddProduct] Found seller via search:', mySeller.id);
          setSellerId(mySeller.id);
          
          // Update user object with seller_id
          const updatedUser = { ...user, seller_id: mySeller.id };
          localStorage.setItem('user', JSON.stringify(updatedUser));
        } else {
          setError('No seller profile found. Please create a seller profile first.');
        }
      } catch (err) {
        console.error('[AddProduct] Failed to fetch seller ID:', err);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Add seller_id
      if (!sellerId) {
        setError('Unable to determine seller ID. Please refresh the page and try again.');
        return;
      }
      console.log('[AddProduct] Using seller_id for product:', sellerId);
      formDataToSend.append('seller_id', sellerId.toString());

      // Add business_idea_id based on selected type
      const businessIdeaId = businessType === 'product' ? '1' : '2';
      formDataToSend.append('business_idea_id', businessIdeaId);
      ...
      additionalImages.forEach((image, index) => {
        formDataToSend.append(`additional_images[${index}]`, image);
      });

      const response = await apiService.addCustomProduct(formDataToSend);
      ...
      let errorMessage = err.message || 'Failed to add product. Please try again.';
      
      // Check for specific price-related errors
      ...
      } else if (errorMessage.includes('Numeric value out of range')) {
        errorMessage = 'One or more numeric values are out of acceptable range. Please check your inputs.';
      }
  ...
  return (
    <div className="add-product-page">
      <div className="add-product-header">
        ...
        <h1 className="add-product-title">Post New Product</h1>
      </div>
